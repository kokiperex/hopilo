...
function addFeature(feature: SectionFeature, section: RouteSection, sectionIndex: number, levelIndex: number, theme: WorldProgressionTheme, collections: Pick<LevelDefinition, 'platforms' | 'ramps' | 'hazards' | 'movingPlatforms' | 'trampolines' | 'conveyors' | 'fans' | 'hammers' | 'obstacles'>): void {
  ...
  if (feature === 'steps') {
    ...
    collections.movingPlatforms.push({ id, position: { x: section.center, y: section.surfaceY + 1.15, z: 0 }, from: { x: section.center - 1.05, y: section.surfaceY + 1.15, z: 0 }, to: { x: section.center + 1.05, y: section.surfaceY + 1.15, z: 0 }, size: { x: 0.36, y: 1.5, z: 2.4 }, cyclesPerSecond: 0.13 + levelIndex * 0.004, color: theme.accent });
