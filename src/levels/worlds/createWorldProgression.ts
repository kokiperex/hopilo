...
function createLevel(world: WorldId, theme: WorldProgressionTheme, beat: LevelBeat, index: number): LevelDefinition {
  ...
  if (world === 'beach' && index === 4) {
    ...
    if (lessonFan) {
      ...
      lessonFan.impulse = 1.8;
      lessonFan.maxSpeed = 6.5;
      lessonFan.lift = 0.3;
    }
  }
...
function addFeature(
  ...
  if (feature === 'steps') {
    ...
    collections.fans.push({ id, position: { x: section.center, y: section.surfaceY + 1.2, z: 0 }, size: { x: Math.min(2.8, section.width * 0.52), y: 2.3, z: 2.55 }, direction: direction > 0 ? 'right' : 'left', impulse: 1.8, maxSpeed: 6.5, lift: 0.35, color: theme.accent });
