...
function validateFan(value: { direction?: unknown; impulse?: unknown; maxSpeed?: unknown; lift?: unknown; id?: unknown; position?: unknown; size?: unknown }, label: string, issues: string[]): void {
  validateSurface(value, label, issues);
  if (value?.direction !== 'left' && value?.direction !== 'right') issues.push(`${label}.direction must be left or right`);
  if (!isPositiveNumber(value?.impulse)) issues.push(`${label}.impulse must be greater than zero`);
  if (value?.maxSpeed !== undefined && !isPositiveNumber(value.maxSpeed)) issues.push(`${label}.maxSpeed must be greater than zero`);
