...
type Screen = 'home' | 'worlds' | 'levels' | 'settings' | 'confirm-reset';
...
export function createNavigation(root: HTMLElement, progress: ProgressStore, callbacks: NavigationCallbacks): Navigation {
  ...
  const render = (): void => {
    ...
    element.innerHTML = screen === 'home' ? homeMarkup() : screen === 'worlds' ? worldsMarkup() : screen === 'levels' ? levelsMarkup(selectedWorld) : screen === 'settings' ? settingsMarkup() : resetMarkup();
    bindActions();
  ...
  const bindActions = (): void => {
    element.querySelectorAll<HTMLButtonElement>('[data-action]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        if (action === 'worlds') setScreen('worlds');
    ...
    return `
      <div class="nav-sky nav-home">
        ...
        <div class="nav-content nav-home-content">
          ...
          <h1>¡Vamos a rodar!</h1>
          <p class="nav-intro">Elige un mundo y lleva la canica azul hasta la bandera.</p>
          <button class="nav-primary-button" type="button" data-action="worlds" data-autofocus>${playIcon}<span>Jugar</span></button>
  ...
  const worldsMarkup = (): string => `
    <div class="nav-sky nav-worlds">
      <header class="nav-header"><button class="nav-back-button" type="button" data-action="home" aria-label="Volver al inicio">←</button><h1>Elige un mundo</h1><span></span></header>
